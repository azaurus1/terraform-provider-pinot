...
`, pinot.URI)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: providerConfig + `data "pinot_tables" "test" {}`,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.pinot_tables.test", "id", "placeholder"), // Test placeholder ID is created
					resource.TestCheckResourceAttr("data.pinot_tables.test", "tables.#", "0"),     // No users when running the docker compose
				),
			},
		},
	})
